...
for s=1:3 % constraining to collaboration/osc or ignore
    ...
    if s == 3
        Rii_2{s} = g.w1*QVP{s}+g.w2*Q + g.r*Qd;
        Ri_i_2{s} = -g.w2*Q;
        R_i_i_2{s} = g.w2*Q;
        ri_2{s} = -g.w1*(g.VP2(1)'*g.giVP{s}*g.Sx...
            +g.VP2(2)'*g.giVP{s}*g.Sy...
            -0.5*g.u0'*QVP0{s}) +g.r*g.u0'*Q0d;

        r_i_2{s} = -g.w2*g.u0'*Q0*0;
        z_2{s} = 1/2*(g.w1*(g.u0'*QVP00{s}*g.u0 ...
            +g.VP2(1)^2 + g.VP2(2).^2 ...
            -2*VP2(1)*g.giVP{s}*g.Sx0*g.u0 ...
            -2*VP2(2)*g.giVP{s}*g.Sy0*g.u0)...
            +g.r*g.u0'*Q00d*g.u0);
    else %% così non consideriamo i casi in cui si inseguono - oscillano
