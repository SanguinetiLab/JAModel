...
% load('FittedParamsv12S.mat')
load('FittedParamsv26H.mat')

% sigmay_other1 = X1(:,3);
% n_groups = length(sigmay_other1);
% sigmax1 = X1(:,2);
% A1 = X1(:,3);
% lambda = 0.1.*ones(1,n_groups);
...
for g = 1:n_groups
    for d = 1:n_sub
        ...
        action_range = -1:0.05:1;%[0:0.01:1];
        ...
        % sensory noise
        sigmay1 = X1(g,3)*eye(xsz1);
        sigmay2 = X2(g,3)*eye(xsz2);
        ...
        % Process oise
        sigmax1 = X1(g,2)*eye(xsz1); %
        sigmax2 = X2(g,2)*eye(xsz2); %

        % Initial state mean and variance
        x01 = X1(g,4).*ones(xsz1,1);
        P01 = X1(g,5).*eye(xsz1);
        x02 = X2(g,4).*ones(xsz2,1);
        P02 = X2(g,5).*eye(xsz2);

        % ACTION CONTROLLER
        % Temperature d0ecreases during experiment
        lambda1_ini = X1(g,6);%.2;
        decay_rate1 = X1(g,7);
        lambda2_ini = X2(g,6);
        decay_rate2 = X2(g,7);
...
subplot(211)
hold on
patch([1:n_blocks n_blocks:-1:1],[mean(cell2mat(pSS'))-std(cell2mat(pSS'))./sqrt(n_groups) fliplr(mean(cell2mat(pSS'))+std(cell2mat(pSS'))./sqrt(n_groups))],purple,'FaceAlpha',0.1,'EdgeAlpha',0)
patch([1:n_blocks n_blocks:-1:1],[mean(cell2mat(pHH'))-std(cell2mat(pHH'))./sqrt(n_groups) fliplr(mean(cell2mat(pHH'))+std(cell2mat(pHH'))./sqrt(n_groups))],orange,'FaceAlpha',0.1,'EdgeAlpha',0)
c(1) = plot(mean(cell2mat(pSS')),'-','color',purple,'linewidth',2)
c(2) = plot(mean(cell2mat(pHH')),'-','color',orange,'linewidth',2)
yline(0.25)
legend(c,{'SS','RR'},'fontname','times')
box off
xlabel('Blocks','fontname','times')
...
subplot(212)
hold on
patch([1:n_blocks n_blocks:-1:1],[mean(cell2mat(pSH'))-std(cell2mat(pSH'))./sqrt(n_groups) fliplr(mean(cell2mat(pSH'))+std(cell2mat(pSH'))./sqrt(n_groups))],light_green,'FaceAlpha',0.1,'EdgeAlpha',0)
patch([1:n_blocks n_blocks:-1:1],[mean(cell2mat(pHS'))-std(cell2mat(pHS'))./sqrt(n_groups) fliplr(mean(cell2mat(pHS'))+std(cell2mat(pHS'))./sqrt(n_groups))],green,'FaceAlpha',0.1,'EdgeAlpha',0)
c(1) = plot(mean(cell2mat(pSH')),'-','color',light_green,'linewidth',2)
c(2) = plot(mean(cell2mat(pHS')),'-','color',green,'linewidth',2)
yline(0.25)
legend(c,{'SR','RS'},'fontname','times')
box off
xlim([0 n_blocks+1])
...
%%
save(fullfile(cd,'results',['sim' num2str(n_sim)]),'SS','HH','SH','HS','pSS','pHH','pSH','pHS','seSS','seHH','seSH','seHS')
